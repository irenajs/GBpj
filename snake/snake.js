...
var barrier_timer = 5000; // Таймер для препятствия
var barrier; //препятствие
var score = 0; // Результат
var elem; //элемент span для счета

function init() {
    prepareGameField(); // Генерация поля
    elem = document.getElementsByTagName('span'); //присвоение span переменной elem
...
function startGame() {
    ...
    barrier = setInterval(createBarrier, barrier_timer);//каждые 5 секунды создается препятствие, они накапливаются на игровом поле
}
/**
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== isBarrier && new_unit !== undefined) {
        ...
        // проверка условия на препятствие
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        createFood();
        score++;
        document.getElementsByTagName('span')[0].innerHTML = score; // запись результата score в span
    }
    return check;
}
